...
export const BentoGridItem = ({
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText('nickyrutto@gmail.com');
  ...
  return (
    <div
      ...
      style={{
        ...
        backgroundColor: 'linear-gradient(90deg, rgba(2,0,36,1) 0%, rgba(10,10,78,1) 35%, rgba(10,30,34,1) 100%)'
      }}
    >
      <div className={`${id === 6 && 'flex justify-center'} h-full`}>
        ...
        {id === 3 && (
          <div className="flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-2">
              <div className="flex flex-col gap-3 lg:gap-8">
                {['Django', 'Flask', 'Python', 'HTML', 'CSS'].map((item) => (
              ...
              <div className="flex flex-col gap-3 lg:gap-8">
              <span className="py-4 px-3 rounded-lg text-center bg-[#10132E]" />
                {['Javascript',  'Postgres', 'MongoDB', 'Nginx', 'Redis'].map((item) => (
                  <span key={item} className="py-2 lg:py-4 lg:px-3 px-3 text-xs lg:text-base opacity-50 lg:opacity-100 rounded-lg text-center bg-[#10132E]">
                    {item}
                  </span>
                ))}
        ...
        {id === 6 && (
          <div className="mt-5 relative">
            ...
            <MagicButton 
              title={copied ? 'Email copied' : 'Copy my emaill'}
              icon={<IoCopyOutline />}
              position="left"
